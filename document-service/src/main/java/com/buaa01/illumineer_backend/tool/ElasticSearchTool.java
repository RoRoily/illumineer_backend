...
public class ElasticSearchTool {
    ...
    public List<String> getMatchedScholar(String name) {
        try {
            ...
            SearchRequest searchRequest = new SearchRequest.Builder()
                    ...
                    .size(10) // 返回最多 10 条结果
                    .build();
            SearchResponse<ElasticSearchScholar> searchResponse = elasticsearchClient.search(searchRequest,
                    ElasticSearchScholar.class);
    ...
    public List<String> getMatchingWord(String word) {
        try {
            ...
            Query query = Query
                    .of(q -> q.simpleQueryString(s -> s.fields("word").query(word).defaultOperator(Operator.And))); // 关键词全匹配
            ...
            SearchRequest searchRequest = new SearchRequest.Builder().index("search_word").query(bool).from(0).size(10)
                    .build();
            SearchResponse<ElasticSearchWord> searchResponse = elasticsearchClient.search(searchRequest,
                    ElasticSearchWord.class);
