        ...
        for (Map<String, String> condition : conditions) {
            ...
            try {
                ...
                if (condition.get("logic").equals("3")) { // NOT
                    query = Query.of(q -> q.bool(b -> b.mustNot(
                            mn -> mn.match(m -> m.field(condition.get("condition")).query(condition.get("keyword"))))));
                } else {
                    query = Query
                            .of(q -> q.match(m -> m.field(condition.get("condition")).query(condition.get("keyword"))));
                }
    ...
    public CustomResponse updatePaperAdoptionStatus(String name, List<Integer> pidsForAdopt) {
        ...
        if (papers == null) {
            ...
            for (Object paperEntity : papers) {
                ...
                if (pidsForAdopt.contains(p.getPid()))
                    p.getAuths().put(name, userClientService.getCurrentUser().getUid());
            }
        }
